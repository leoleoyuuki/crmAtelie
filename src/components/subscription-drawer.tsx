  ...
  return (
    <div className="inline-flex items-center bg-muted/50 rounded-full p-1 shadow-inner relative border border-black/5 w-max">
      <button
        ...
        className={cn(
          "px-6 py-2 rounded-full text-sm font-bold transition-colors duration-200 relative z-10",
          selected === 'mensal'
      ...
      <button
        ...
        className={cn(
          "px-6 py-2 rounded-full text-sm font-bold transition-colors duration-200 relative z-10 flex items-center gap-1.5",
          selected === 'anual'
            ? "text-primary-foreground"
            : "text-muted-foreground hover:text-foreground"
        )}
      >
        Anual
        {selected !== 'anual' && (
          <span className="absolute -top-2 -right-2 bg-[#10b981] text-white text-[9px] px-1.5 py-0.5 rounded-full font-black shadow-lg">
            -R$200
          </span>
        )}
      </button>
      ...
      <div 
        className={cn(
          "absolute inset-y-1 rounded-full bg-primary shadow-lg transition-all duration-300 ease-out z-0",
...
function CouponCard({ code, label, description }: { code: string; label: string; description: string }) {
  ...
  return (
    <div className="group relative bg-gradient-to-br from-muted/60 to-muted/30 border border-primary/15 rounded-2xl p-5 hover:border-primary/30 transition-colors duration-200">
      ...
      <div className="flex items-center gap-3">
        ...
        <Button
          ...
          className={cn(
            "h-12 w-12 rounded-xl shrink-0 transition-colors duration-200 border-2",
            copied
      ...
      {copied && (
        <p className="text-xs text-emerald-600 font-bold mt-2 text-center">✓ Código copiado!</p>
      )}
    </div>
  );
}
...
export function SubscriptionDrawer({ profile, open, onOpenChange }: { profile: UserProfile | null, open?: boolean, onOpenChange?: (open: boolean) => void }) {
  ...
  return (
      <Sheet open={open} onOpenChange={(v) => {
        ...
        <SheetContent side="bottom" className="p-0 border-t border-primary/20 rounded-t-3xl sm:rounded-none sm:rounded-t-[40px] w-full max-w-none sm:mx-0 sm:right-auto sm:left-0 sm:translate-x-0 h-auto max-h-[90vh] overflow-y-auto bg-background shadow-[0_-20px_50px_-20px_rgba(0,0,0,0.1)]">
          <div className="relative overflow-hidden w-full">
            ...
            <div className="max-w-7xl mx-auto px-6 sm:px-12 py-8 lg:py-12 relative z-10 w-full">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
                
                {/* Feedback side (Hidden on Mobile) */}
                <div className="hidden lg:flex flex-col gap-6 relative z-10 lg:border-r lg:border-primary/10 pr-0 lg:pr-10">
                   <div>
                      <h3 className="text-2xl font-headline font-black mb-2 text-foreground tracking-tight">O que dizem nossas clientes</h3>
                      <p className="text-sm text-muted-foreground font-medium">Junte-se a centenas de ateliês que já organizaram suas vendas.</p>
                   </div>
                   <div className="space-y-3">
                     {feedbackMessages.map((fb, i) => (
                        <div key={i} className="bg-muted/40 hover:bg-muted/60 transition-colors duration-200 p-4 rounded-2xl relative shadow-sm border border-black/5">
                           <div className="flex items-center gap-3 mb-2">
                              <div className="bg-primary/10 w-8 h-8 rounded-full flex items-center justify-center font-black text-primary text-sm shadow-inner">
                               {fb.name.charAt(0)}
                             </div>
                ...
                {/* Pricing side */}
                <div className="space-y-5 relative z-10 lg:pl-6">
                  <SheetHeader className="text-center lg:text-left space-y-2 mb-4 relative z-10">
                    <div className="bg-primary/10 w-12 h-12 rounded-2xl flex items-center justify-center mx-auto lg:mx-0 mb-2">
                      <Sparkles className="h-6 w-6 text-primary fill-primary/30" />
                    </div>
                    <SheetTitle className="font-headline text-2xl md:text-3xl font-black tracking-tight text-foreground">
                      AtelierFlow <span className="text-primary">Plus</span>
                    </SheetTitle>
                    <SheetDescription className="text-sm font-medium text-foreground/70 max-w-md mx-auto lg:mx-0">
                    ...
                    {billingCycle === 'mensal' ? (
                      ...
                      <div className="flex flex-col items-center lg:items-start">
                        ...
                        <div className="flex items-center justify-center lg:justify-start gap-4">
                          ...
                          {/* Impressora flutuante ao lado do preço */}
                          <div className="relative shrink-0 flex items-center justify-center">
                            {/* Sombra no chão */}
                            <div
                              className="absolute bottom-0 left-1/2 -translate-x-1/2 w-16 h-3 rounded-full pointer-events-none"
                  ...
                  {billingCycle === 'anual' && (
                    <div className="py-0">
                      <div className="space-y-0.5">
                        <p className="text-[10px] font-black uppercase tracking-widest text-primary">🎁 Brinde Exclusivo</p>
                        ...
                        <p className="text-xs text-muted-foreground font-medium leading-snug">
                          Ganhe uma impressora de 58mm para imprimir tickets e comprovantes pelo sistema.
                        </p>
                      </div>
                    </div>
                  )}

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pb-1 max-w-lg mx-auto lg:mx-0">
                    {features.map((feature, i) => (
                      <div key={i} className="flex items-center gap-2">
                        <div className="bg-primary/10 p-0.5 rounded-lg shrink-0">
                          <Check className="h-3.5 w-3.5 text-primary stroke-[3px]" />
                        </div>
                        <span className="text-xs text-foreground/90 font-bold">{feature}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  {showCouponScreen && profile?.stripeCustomerId && (
                    <div className="space-y-5 mt-2">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        <Button 
                          size="lg"
                          className="flex-1 h-14 rounded-2xl font-black shadow-lg bg-primary hover:bg-primary/95"
                  ...
                  {!showCouponScreen && (
                    <Button 
                      size="lg"
                      className="w-full lg:max-w-md h-14 text-base rounded-2xl font-black shadow-lg group bg-primary hover:bg-primary/95 mt-2"
                      onClick={() => {
