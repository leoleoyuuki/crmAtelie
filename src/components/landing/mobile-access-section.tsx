...
import { Sparkles, Wifi, Signal, BatteryFull, ArrowRight, Smartphone, CheckCircle2 } from 'lucide-react';
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <p className="text-lg text-muted-foreground leading-relaxed max-w-xl font-medium">
              Adicione o AtelierFlow à tela inicial do seu celular e tenha a experiência de um aplicativo nativo. Gerencie pedidos e envie comprovantes direto do balcão.
            </p>

            <div className="space-y-4 max-w-md">
                {[
                    { icon: Smartphone, title: "Instalação Simples", desc: "Sem baixar pela loja, direto do navegador." },
                    { icon: CheckCircle2, title: "Acesso Offline", desc: "Consulte seus contatos mesmo sem internet." }
                ].map((item, i) => (
            ...
            <div className="flex items-center gap-2 text-primary font-black uppercase tracking-widest text-[10px] cursor-pointer group w-fit transition-all">
                Saiba como instalar no seu celular
          ...
          >
            {/* Mockup de iPhone com Animação de Flutuação */}
            <motion.div 
                animate={{ y: [0, -20, 0] }}
            ...
            >
                ...
                <div className="rounded-[2.8rem] overflow-hidden w-full h-full bg-background relative flex flex-col">
                    ...
                    <div className="flex-1 p-5 space-y-6">
                        ...
                        <div className="h-24 w-full bg-primary rounded-2xl shadow-lg p-4 flex flex-col justify-end">
                            <div className="h-1.5 w-12 bg-white/30 rounded-full mb-2" />
                            <div className="h-3 w-24 bg-white/80 rounded-full" />
                        </div>

                        <div className="grid grid-cols-2 gap-3">
                            <div className="h-20 bg-muted/40 rounded-2xl border border-dashed border-muted-foreground/20 flex flex-col items-center justify-center gap-2">
                                <div className="h-6 w-6 rounded-full bg-primary/10" />
                                <div className="h-1.5 w-10 bg-muted-foreground/30 rounded-full" />
                            </div>
                            <div className="h-20 bg-muted/40 rounded-2xl border border-dashed border-muted-foreground/20 flex flex-col items-center justify-center gap-2">
                                <div className="h-6 w-6 rounded-full bg-secondary/10" />
                                <div className="h-1.5 w-10 bg-muted-foreground/30 rounded-full" />
                            </div>
                        </div>
                        ...
                        <div className="space-y-3 pt-2">
                            {[1, 2, 3].map(i => (
                                <div key={i} className="h-12 w-full bg-white/50 dark:bg-white/5 border rounded-xl flex items-center px-3 gap-3">
                                    <div className="h-6 w-6 rounded-lg bg-muted" />
                                    <div className="space-y-1.5">
                                        <div className="h-2 w-20 bg-muted rounded-full" />
                                        <div className="h-1 w-12 bg-muted/60 rounded-full" />
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            {/* Decorative Shine Over the phone */}
